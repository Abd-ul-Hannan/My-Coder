...
const VULN_PATTERNS: VulnerabilityPattern[] = [
  // XSS
 ...
 */
export async function scanWorkspaceSecurity(rootPath: string): Promise<SecurityReport> {
  ...
  try {
    const pkg = JSON.parse(await fs.readFile(packagePath, 'utf-8')) as Record<string, unknown>;
...
function scanForSecrets(content: string, filePath: string, out: SecurityFinding[]): void {
...
function scanForVulnerabilities(content: string, filePath: string, out: SecurityFinding[]): void {
...
async function gatherFiles(root: string, exts: Set<string>, ignoreDirs: Set<string>): Promise<string[]> {
  ...
  async function walk(dir: string): Promise<void> {
    try {
      ...
      for (const e of entries) {
        ...
        else if (exts.has(path.extname(e.name))) results.push(full);
      }
    } catch { /* skip */ }
  }
  await walk(root);
...
function formatSummary(findings: SecurityFinding[], fileCount: number): string {
  if (!findings.length) return `✅ No security issues found in ${fileCount} files.`;
