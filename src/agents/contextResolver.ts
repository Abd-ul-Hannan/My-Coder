 ...
 */
export function hasContextReferences(message: string): boolean {
  return (
    AT_FILE_PATTERN.test(message) ||
 ...
 */
export function detectAtReferencePrefix(inputSoFar: string): string | null {
...
async function getWorkspaceStructure(rootPath: string, depth = 3): Promise<string> {
  const lines: string[] = [];
  await walkDir(rootPath, '', depth, lines);
  return lines.slice(0, 60).join('\n');
}

async function walkDir(dir: string, prefix: string, remaining: number, out: string[]): Promise<void> {
...
async function findSymbolInWorkspace(symbolName: string, rootPath: string): Promise<string | null> {
...
async function getAllFilesRecursive(dir: string, extensions: string[]): Promise<string[]> {
...
async function extractUrlContent(url: string, tavilyKey: string): Promise<string> {
  try {
    ...
    if (!response.ok) return `[Could not extract: HTTP ${response.status}]`;
