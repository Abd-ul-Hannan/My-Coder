...
export class PlannerAgent {
  ...
  async generatePlan(interview: ProjectInterview): Promise<DevelopmentPlan> {
   ...
   */
  async refinePlan(plan: DevelopmentPlan, feedback: string): Promise<DevelopmentPlan> {
    const prompt =
    ...
    const refined = this.parsePlan(response.content, { projectName: plan.projectName } as ProjectInterview);
  ...
  sortFilesByDependency(files: FileSpec[]): FileSpec[] {
    const configs = files.filter((f) =>
      f.path.endsWith('.json') || f.path.endsWith('.yaml') ||
  ...
  private buildPrompt(interview: ProjectInterview): string {
  ...
  private parsePlan(raw: string, interview: Pick<ProjectInterview, 'projectName'>): DevelopmentPlan {
    const cleaned = raw.replace(/```json\s*/gi, '').replace(/```\s*/gi, '').trim();
    try {
      const parsed = JSON.parse(cleaned) as DevelopmentPlan;
      if (!parsed.projectName) parsed.projectName = interview.projectName;
      return parsed;
    } catch (err) {
  ...
  private validatePlan(plan: DevelopmentPlan): void {
    const required: (keyof DevelopmentPlan)[] = ['projectName', 'files', 'dependencies', 'initCommands', 'folderStructure'];
    for (const field of required) {
      if (!plan[field]) throw new Error(`PlannerAgent: plan missing required field "${field}".`);
    }
