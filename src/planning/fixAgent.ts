...
import { BuildResult, BuildError, WorkspaceAnalysis } from '../types';
 ...
 */
export async function analyzeBuildErrorsAndFix(
...
function formatError(error: BuildError): string {
  ...
  if (error.file) parts.push(`  File: ${error.file}${error.line ? `:${error.line}` : ''}`);
 ...
 */
function buildFilesSection(errors: BuildError[], fileContents: Map<string, string>): string {
...
function parseFixPlan(raw: string): FixPlan {
  ...
  try {
    ...
    if (match) {
      ...
      catch { throw new Error(`FixAgent: could not parse fix plan JSON.\nRaw: ${cleaned.slice(0, 400)}`); }
    } else {
  ...
  return {
    ...
    fixes: obj['fixes'] as Record<string, string>,
  };
}
