...
import { DevelopmentPlan, DeploymentTarget, ProjectInterview } from '../types';
...
export class DeploymentAgent {
  ...
  private async generateVercel(interview: ProjectInterview, plan: DevelopmentPlan): Promise<DeploymentFiles> {
  ...
  private async generateDocker(interview: ProjectInterview, plan: DevelopmentPlan): Promise<DeploymentFiles> {
...
volumes:
  ...
  private async generateRailway(interview: ProjectInterview, plan: DevelopmentPlan): Promise<DeploymentFiles> {
  ...
  private async generateFly(interview: ProjectInterview, plan: DevelopmentPlan): Promise<DeploymentFiles> {
...
`;
  ...
  private async generateAWS(interview: ProjectInterview, plan: DevelopmentPlan): Promise<DeploymentFiles> {
    const response = await this.aiService.complete(
      [{ role: 'user', content: `Generate AWS deployment config for: ${interview.projectName} (${interview.framework}, ${interview.language}). Include: Dockerfile, buildspec.yml for CodeBuild, and basic CloudFormation template.` }],
      'You are an AWS DevOps expert. Return only the requested files as a JSON object: { "Dockerfile": "...", "buildspec.yml": "...", "cloudformation.yaml": "..." }. No markdown.',
    );
    try {
  ...
  private async generateGenericCI(interview: ProjectInterview, plan: DevelopmentPlan): Promise<DeploymentFiles> {
    return {
  ...
  private githubActionsCI(interview: ProjectInterview, target: string, deploySteps: string): string {
