...
import { InlineChatProvider } from './ui/inlineChatProvider';
import { BottomPanelProvider } from './ui/bottomPanel';
import { registerInlineCompletions } from './tools/inlineCompletionProvider';
import { AIService } from './ai/aiService';
import { initializeRulesDirectory } from './tools/rulesLoader';

export async function activate(context: vscode.ExtensionContext): Promise<void> {
  console.log('[MY Coder] Activating...');

  // ── Shared AI service getter (lazy, cached per-session) ───────────────────
  let cachedAI: AIService | null = null;
  const getAIService = async (): Promise<AIService | null> => {
    try {
      if (!cachedAI) cachedAI = await AIService.createFromVSCodeConfig(context);
      return cachedAI;
    } catch { return null; }
  };
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration(e => {
      if (e.affectsConfiguration('myCoder')) cachedAI = null;
    }),
  );

  // ── Register ChatPanel as a persistent sidebar WebviewViewProvider ─────────
  const chatPanelProvider = new ChatPanel(context);
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(ChatPanel.viewType, chatPanelProvider, {
  ...
  const postToPanel = (msg: any, delayMs = 400): void => {
    setTimeout(() => {
      try {
  ...
  registerInlineCompletions(context, getAIService);

  // ── Inline chat provider (Ctrl+I / Cmd+I) ─────────────────────────────────
  const inlineChatProvider = new InlineChatProvider(getAIService);
  context.subscriptions.push(
    vscode.commands.registerCommand('my-coder.inlineChat', async () => {
      const editor = vscode.window.activeTextEditor;
      if (editor) await inlineChatProvider.showInlineChat(editor);
    }),
  );

  // ── Bottom panel with Problems/Actions/Terminal ────────────────────────────
  const bottomPanel = new BottomPanelProvider(context, getAIService);
  context.subscriptions.push(
    vscode.commands.registerCommand('my-coder.showBottomPanel', () => bottomPanel.show()),
  );

  // ── Panel commands ────────────────────────────────────────────────────────
  context.subscriptions.push(
    vscode.commands.registerCommand('my-coder.openPanel', () => {
  ...
  // ── Utility commands ─────────────────────────────────────────────────────
    vscode.commands.registerCommand('my-coder.initRules', async () => {
      ...
      if (!folders) { vscode.window.showErrorMessage('Open a workspace first.'); return; }
      const p = await initializeRulesDirectory(folders[0].uri.fsPath);
  ...
  // ── First-run welcome ─────────────────────────────────────────────────────
  if (!context.globalState.get<boolean>('my-coder.activated')) {
    await context.globalState.update('my-coder.activated', true);
    vscode.window.showInformationMessage(
      '🤖 MY Coder is ready! Set your AI provider key to get started.',
      'Open Panel', 'Set API Key',
    ).then(choice => {
      if (choice === 'Open Panel')  vscode.commands.executeCommand('my-coder.openPanel');
      if (choice === 'Set API Key') vscode.commands.executeCommand('my-coder.setApiKey');
    });
  }
...
export function deactivate(): void { console.log('[MY Coder] Deactivated.'); }
