export type AIProvider = 'openai' | 'anthropic' | 'groq' | 'gemini' | 'mistral' | 'ollama' | 'openrouter' | 'perplexity' | 'custom';
 ...
 */
export type AppCategory = 'ecommerce' | 'dashboard' | 'blog' | 'ai-tool' | 'saas' | 'portfolio' | 'game' | 'api-service' | 'mobile-app' | 'cli-tool' | 'custom';
export type Framework = 'react' | 'nextjs' | 'vue' | 'nuxt' | 'angular' | 'svelte' | 'express' | 'fastify' | 'nestjs' | 'hono' | 'flutter' | 'react-native' | 'expo' | 'electron' | 'tauri' | 'none';
export type Database = 'postgresql' | 'mysql' | 'mongodb' | 'sqlite' | 'supabase' | 'firebase' | 'redis' | 'none';
export type DeploymentTarget = 'vercel' | 'netlify' | 'aws' | 'gcp' | 'azure' | 'docker' | 'railway' | 'fly' | 'none';
 ...
 */
export type InterviewPhase = 'intent' | 'refinement' | 'stack-selection' | 'database' | 'auth' | 'deployment' | 'features' | 'confirmation' | 'finalized';
...
export type MessageType = 'text' | 'plan' | 'code' | 'diff' | 'build-result' | 'error' | 'progress' | 'interview';
...
export type WebviewMessageType = 'sendMessage' | 'startNewApp' | 'startWorkOnProject' | 'signIn' | 'signOut' | 'loadHistory' | 'loadSession' | 'deleteSession' | 'renameSession' | 'clearHistory' | 'setApiKey' | 'updateSettings' | 'testConnection' | 'detectOllamaModels' | 'cancelOperation' | 'applyDiff' | 'rejectDiff' | 'runSecurityScan' | 'getTodos' | 'completeTodo' | 'deleteTodo' | 'getSlashCommands' | 'webSearch' | 'fixActiveFile' | 'explainActiveFile' | 'reviewActiveFile';
/** Messages FROM the extension TO the webview */
export type ExtensionMessageType = 'message' | 'progress' | 'plan' | 'diff' | 'buildResult' | 'historyList' | 'sessionLoaded' | 'authStatus' | 'providerStatus' | 'connectionTestResult' | 'ollamaModels' | 'error' | 'ready' | 'operationComplete';
