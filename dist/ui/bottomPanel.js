...
exports.BottomPanelProvider = void 0;
const vscode = __importStar(require("vscode"));
class BottomPanelProvider {
    context;
    getAI;
    panel;
    terminal;
    diagnosticsWatcher;
    constructor(context, getAI) {
    ...
    show() {
        ...
        this.panel = vscode.window.createWebviewPanel('myCoderBottomPanel', 'MY Coder — Actions', { viewColumn: vscode.ViewColumn.Two, preserveFocus: true }, { enableScripts: true, retainContextWhenHidden: true });
        this.panel.webview.html = this.getHtml();
        this.panel.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.type) {
                case 'runAction':
                    await this.handleAction(msg.payload);
                    break;
                case 'loadProblems':
                    this.sendProblems();
                    break;
                case 'openTerminal':
                    this.openTerminal(msg.payload);
                    break;
                case 'fixProblem':
                    await this.fixProblem(msg.payload);
                    break;
...
exports.BottomPanelProvider = BottomPanelProvider;
