...
class SupabaseAgent {
     ...
     */
    async generateFullSetup(interview) {
        const projectDesc = `
...
`;
    ...
    async generateSchema(projectDesc) {
        const response = await this.aiService.complete([{ role: 'user', content: `Generate Supabase schema for:\n${projectDesc}` }], SCHEMA_PROMPT);
        try {
    ...
    async generateAuthHelpers() {
        const response = await this.aiService.complete([{ role: 'user', content: 'Generate Supabase SSR auth helpers for Next.js 14 app router.' }], AUTH_HELPERS_PROMPT);
        return response.content.replace(/```typescript\s*/gi, '').replace(/```\s*/gi, '').trim();
    }
    async generateEdgeFunctions(interview) {
        const functions = [];
        ...
        for (const req of requests) {
            const response = await this.aiService.complete([{ role: 'user', content: `Generate a Supabase Edge Function: ${req.prompt}` }], EDGE_FUNCTION_PROMPT);
            functions.push({ name: req.name, content: response.content.replace(/```typescript\s*/gi, '').replace(/```\s*/gi, '').trim() });
        }
        return functions;
    }
    // ─── Type-safe client ─────────────────────────────────────────────────────
    generateTypesSafeClient() {
...
//# sourceMappingURL=supabaseAgent.js.map