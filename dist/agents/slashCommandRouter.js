...
exports.buildAgentTask = buildAgentTask;
exports.getSlashCommandList = getSlashCommandList;
...
function parseSlashCommand(input) {
    ...
    const commands = Object.keys(COMMAND_META);
 ...
 */
function buildAgentTask(command, request) {
...
function getSlashCommandList() {
    return Object.entries(COMMAND_META).map(([command, meta]) => ({ command, description: meta.description }));
}
/**
 ...
 */
async function executeSlashCommand(task, fileContext, aiService, onChunk) {
    const userMessage = [
    ...
    let content;
    if (onChunk) {
        content = await aiService.streamComplete([{ role: 'user', content: userMessage }], task.systemPrompt, onChunk);
    }
    else {
        const response = await aiService.complete([{ role: 'user', content: userMessage }], task.systemPrompt);
...
//# sourceMappingURL=slashCommandRouter.js.map