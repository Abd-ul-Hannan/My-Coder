...
class CodeAgent {
     ...
     */
    async generateFile(request, onChunk) {
        const systemPrompt = this.buildSystemPrompt(request.plan, request.interview);
        const userPrompt = this.buildUserPrompt(request);
        const content = await this.aiService.streamComplete([{ role: 'user', content: userPrompt }], systemPrompt, onChunk ?? (() => undefined));
     ...
     */
    async generatePatch(filePath, originalContent, instruction, projectContext) {
    ...
    // ─── Private ─────────────────────────────────────────────────────────────
    buildSystemPrompt(plan, interview) {
