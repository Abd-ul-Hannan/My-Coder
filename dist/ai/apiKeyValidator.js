...
exports.humanizeApiError = humanizeApiError;
// ─── Prefix → Provider Map ────────────────────────────────────────────────────
/**
 ...
 */
const PROVIDERS_WITH_FLEXIBLE_KEYS = new Set([
    'mistral',
...
function validateApiKey(apiKey, provider) {
    ...
    if (detectedProvider !== null && detectedProvider !== provider) {
        ...
        return {
            ...
            detectedProvider,
            mismatch: true,
            message: `This API key looks like a ${providerNames[detectedProvider]} key ` +
 ...
 */
function humanizeApiError(error, provider) {
    ...
    if (lower.includes('401') || lower.includes('incorrect api key') || lower.includes('invalid api key')) {
        ...
        return (`Invalid API key or wrong provider selected.\n` +
...
// ─── Private Helpers ──────────────────────────────────────────────────────────
function getPrefixForProvider(provider) {
