...
const FRAMEWORK_OPTIONS_BY_TYPE = {
    web: [
...
const FEATURE_OPTIONS_BY_CATEGORY = {
    ecommerce: [
    ...
    getCurrentQuestion() {
        ...
        switch (this.phase) {
            ...
            case 'refinement':
                return {
                    phase: 'refinement',
                    question: `I understood: **${this.inferred?.reasoning ?? 'your project'}**\n\n` +
        ...
        switch (this.phase) {
            ...
            case 'refinement': {
                ...
                if (text === 'yes' || text === 'y' || text === 'correct' || text === 'looks good') {
                    ...
                    this.applyRefinementText(answer);
                    this.phase = 'stack-selection';
                }
                break;
            }
            case 'stack-selection':
                this.answers.framework = answer;
                this.phase = 'database';
                break;
            case 'database':
                this.answers.database = answer;
                this.phase = 'auth';
                break;
            case 'auth':
                if (answer === 'none') {
                    ...
                    this.answers.authProvider = answer;
                }
                this.phase = 'deployment';
                break;
            case 'deployment':
                this.answers.deploymentTarget = answer;
                this.phase = 'features';
                break;
            case 'features': {
    ...
    applyRefinementText(text) {
        ...
        for (const [keyword, fw] of frameworkKeywords) {
            if (lower.includes(keyword)) {
                this.answers.framework = fw;
                break;
            }
        }
        if (lower.includes('mobile')) {
            this.answers.projectType = 'mobile';
            this.answers.framework = 'expo';
        }
        if (lower.includes('backend') || lower.includes('api')) {
            this.answers.projectType = 'backend';
        }
        if (lower.includes('postgres')) {
            this.answers.database = 'postgresql';
        }
        if (lower.includes('mongo')) {
            this.answers.database = 'mongodb';
        }
        if (lower.includes('supabase')) {
            this.answers.database = 'supabase';
        }
    }
...
//# sourceMappingURL=interviewStateMachine.js.map